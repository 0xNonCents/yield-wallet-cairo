...
async def account_factory(get_starknet):
    async def _account_factory(guardian):
        ...
        account = await deploy(starknet, "./ArgentAccount.cairo", [account_signer.public_key, guardian])
        return account
...
async def guardian_factory(get_starknet):
    starknet = get_starknet
    guardian1 = await deploy(starknet, "./guardians/SCSKGuardian.cairo", [guardian_1_key.public_key])
    guardian2 = await deploy(starknet, "./guardians/USDKGuardian.cairo")
...
async def test_scsk_guardian(account_factory, guardian_factory, dapp_factory):
    ...
    await assert_revert(
        sender.send_transaction(dapp.contract_address, 'set_number', [
                                67], [account_signer, guardian_1_key])
    )
...
async def test_guardian_suite(account_factory, guardian_factory, dapp_factory):
    ...
    await assert_revert(
        sender.send_transaction(account.contract_address,
                                'trigger_escape_signer', [], [guardian_2_key_1])
    )
    ...
    assert (escape.active_at == (127 + ESCAPE_SECURITY_PERIOD)
            and escape.caller == guardian2.contract_address)
