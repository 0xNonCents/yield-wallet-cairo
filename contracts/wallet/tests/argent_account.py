...
async def account_factory(get_starknet):
    ...
    guardian = await deploy(starknet, "./guardians/SCSKGuardian.cairo", [guardian_signer.public_key])
    account = await deploy(starknet, "./ArgentAccount.cairo", [signer.public_key, guardian.contract_address])
...
async def dapp_factory(get_starknet):
    starknet = get_starknet
    dapp = await deploy(starknet, "./TestDapp.cairo")
    return dapp
...
async def test_call_dapp_with_guardian(account_factory, dapp_factory):
    ...
    await assert_revert(
        sender.send_transaction(dapp.contract_address, 'set_number', [
                                47], [signer, guardian_signer], nonce=3)
    )

    # should revert with the wrong signer
    await assert_revert(
        sender.send_transaction(dapp.contract_address, 'set_number', [
                                47], [wrong_signer, guardian_signer])
    )

    # should revert with the wrong guardian key
    await assert_revert(
        sender.send_transaction(dapp.contract_address, 'set_number', [
                                47], [signer, wrong_guardian_signer])
    )

    # should fail with only 1 signer
    await assert_revert(
        sender.send_transaction(dapp.contract_address,
                                'set_number', [47], [signer])
    )
...
async def test_call_dapp_no_guardian(get_starknet, dapp_factory):
    starknet = get_starknet
    account_no_guardian = await deploy(starknet, "./ArgentAccount.cairo", [signer.public_key, 0])
    dapp = dapp_factory
    ...
    await assert_revert(
        sender.send_transaction(
            account_no_guardian.contract_address, 'trigger_escape_guardian', [], [signer])
    )
...
async def test_change_signer(account_factory):
    ...
    # should revert with the wrong signer
    await assert_revert(
        sender.send_transaction(account.contract_address, 'change_signer', [
                                new_signer.public_key], [wrong_signer, guardian_signer])
    )
...
async def test_change_guardian(account_factory):
    ...
    # should revert with the wrong signer
    await assert_revert(
        sender.send_transaction(account.contract_address, 'change_guardian', [
                                new_guardian.public_key], [wrong_signer, guardian_signer])
    )

    # should revert with the wrong guardian signer
    await assert_revert(
        sender.send_transaction(account.contract_address, 'change_guardian', [
                                new_guardian.public_key], [signer, wrong_guardian_signer])
    )
...
async def test_trigger_escape_guardian(account_factory):
    ...
    escape = (await account.get_escape().call()).result
    assert (escape.active_at == (127 + ESCAPE_SECURITY_PERIOD)
            and escape.caller == signer.public_key)
...
async def test_trigger_escape_signer(account_factory):
    ...
    escape = (await account.get_escape().call()).result
    assert (escape.active_at == (127 + ESCAPE_SECURITY_PERIOD)
            and escape.caller == guardian.contract_address)
...
async def test_escape_guardian(account_factory):
    ...
    await assert_revert(
        sender.send_transaction(account.contract_address, 'escape_guardian', [
                                new_guardian.public_key], [signer])
    )
...
async def test_escape_signer(account_factory):
    ...
    # should fail to escape before the end of the period
    await assert_revert(
        sender.send_transaction(account.contract_address, 'escape_signer', [
                                new_signer.public_key], [guardian_signer])
    )
...
async def test_cancel_escape(account_factory):
    ...
    await assert_revert(
        sender.send_transaction(account.contract_address,
                                'cancel_escape', [], [signer])
    )
